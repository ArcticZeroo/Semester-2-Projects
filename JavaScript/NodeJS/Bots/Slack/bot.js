...
function getMentionsList(){
    fs.readFile('./mentions.json', (error, data)=>{
        return JSON.parse(data);
    });
}

function updateMentions(){
    mentions = JSON.parse(fs.readFileSync('./mentions.json'));
}

var mentions = getMentionsList();
var prefix;
...
function mentionUsers(message){
    updateMentions();
    var tempArray = mentions.users[type].join(" ").split(" ");
    var senderIndex = tempArray.indexOf("<@" + getUserMention(message.user) + ">");
    if(senderIndex > -1){
        tempArray.splice(senderIndex, 1);
    }
    var toMention = tempArray.join(", ");

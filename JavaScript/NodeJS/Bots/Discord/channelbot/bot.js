...
function consolePing(){
  log("I'm still here! I have been online for " + (bot.uptime/60000) + " minutes.");
}
...
function purgeChannels(server){
  ...
  for(var i = 0; i < server_channels.length; i++){
    ...
    if(!(active_channel["name"] in channels) && active_channel.members.length == 0){
      ...
      log("Deleted channel " + active_channel["name"]);
      count++;
...
bot.on("messageDeleted", function(message, channel){
  log("Message sent by " + message.author.name + " was deleted from " + channel.name);
});
...
bot.on("channelUpdated", function(oldchannel, newchannel){
  if(oldchannel.name != newchannel.name && oldchannel.name in channels){
    log("A channel's name has changed!")
    channels[newchannel.name] = channels[oldchannel.name];
    delete channels[oldchannel.name];
    updateConfig();
  }
});
